...
async function generateInvoice(
  ...
  const itemLines = localItems.map((item, i) =>
    `• ${item.productName} (Size ${item.sizeNumber}) x${lines[i].qty} — $${(lines[i].qty * lines[i].price).toFixed(2)}`
  ).join("\n");
  const waText = encodeURIComponent(
    `🛒 *Gucha Sneakers* — Resumen de compra\n\n${buyer ? `👤 ${buyer}\n` : ""}${itemLines}\n\n💰 *Total: $${total.toFixed(2)}*\n\n¡Gracias por tu compra! 🙏`
  );
  const waUrl = `https://wa.me/?text=${waText}`;

  const rows = localItems.map((item, i) => `
    <tr>
...
export default function BulkSaleModal({ items, onSell, onClose }: Props) {
  const [localItems, setLocalItems] = useState<AdminSelectedSize[]>(items);
  ...
  function removeItem(sizeId: number) {
    const idx = localItems.findIndex((it) => it.sizeId === sizeId);
    if (idx === -1) return;
  ...
  const lines = localItems.map((item, i) => ({
  ...
  function updQty(i: number, v: number) {
    setQtys((prev) => { const n = [...prev]; n[i] = Math.max(1, Math.min(v, localItems[i].maxQty)); return n; });
  }
  ...
  async function handleSell(withInvoice: boolean) {
    if (saving) return;
    setSaving(true);
    await onSell(localItems.map((item, i) => ({
    ...
    if (withInvoice) await generateInvoice(localItems, lines, buyer.trim(), note.trim());
    setSaving(false);
    onClose();
  }
  ...
  if (localItems.length === 0) {
    return (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4"
        ...
        <div className="bg-[#111] border border-gucha-border rounded-2xl w-full max-w-md shadow-card animate-fade-up flex flex-col items-center py-12 px-6 gap-4"
          onClick={(e) => e.stopPropagation()}>
          <p className="text-5xl opacity-30">🛒</p>
          <p className="text-white font-semibold text-[14px]">No hay items en la venta</p>
          <p className="text-gucha-muted text-[12px] text-center">Eliminaste todos los items. Cerrá y seleccioná de nuevo.</p>
          <button onClick={onClose}
            className="mt-2 px-6 py-2.5 rounded-xl border border-gucha-border text-gucha-muted text-[12px] font-semibold hover:text-white transition-colors">
            Cerrar
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gucha-border/60 flex-shrink-0">
          <div>
            ...
            <p className="text-[15px] font-black text-white">Registrar venta
              <span className="ml-2 text-[11px] font-normal text-gucha-muted">({localItems.length} {localItems.length === 1 ? "item" : "items"})</span>
            </p>
          </div>
            ...
            <div key={item.sizeId} className="bg-[#0d0d0d] border border-gucha-border rounded-xl p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <span className="px-2 py-0.5 bg-gucha-dark border border-gucha-border rounded-lg text-[11px] font-bold text-gucha-muted">
                    Size {item.sizeNumber}
                  </span>
                  <button
                    onClick={() => removeItem(item.sizeId)}
                    title="Quitar item"
                    className="w-6 h-6 flex items-center justify-center rounded-lg bg-gucha-dark border border-gucha-border text-gucha-muted hover:text-gucha-red hover:border-gucha-red/50 transition-colors text-[10px] font-bold"
                  >
                    ✕
                  </button>
                </div>
